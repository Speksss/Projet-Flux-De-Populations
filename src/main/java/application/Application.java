...
public class Application {
    ...
    private void analyseCapteurs() {
        ...
        for(Event e : eventList) {
            ...
            if((type.getName().equals("TemperatureCritique")) || (type.getName().equals("TemperatureElevee")) || (type
                    .getName()
                    .equals("TemperatureFaible")))
            {
                boolean res = false;
                for(Capteur c : capteurService.getCapteurByArea(e.getArea())) {
                    ...
                    for(String nomCapteur : jobj.keySet()) {
                        double temperature = Double.parseDouble(
                                (String) jobj.getJSONObject(nomCapteur).get("Temperature"));
                        if(((type.getName().equals("TemperatureCritique")) && (temperature > 50.0)) || ((type
                                .getName()
                                .equals("TemperatureElevee")) && (temperature > 30.0) && (temperature < 50.0)) || ((type
                                .getName()
                                .equals("TemperatureFaible")) && (temperature < 15.0)))
                        {
                            res = true;
                            break;
                        }
    ...
    private void detectUsersOutOfZone() {
        ...
        for(UserLocation ul : userLocations) {
            Area uphf = new Area();
            uphf.setCoordinates(
                    "{\"x1\":\"50.320309\",\"y1\":\"3.513892\",\"x2\":\"50.329210\",\"y2\":\"3.518154\",\"x3\":\"50" +
                            ".328642\",\"y3\":\"3.509260\",\"x4\":\"50.321190\",\"y4\":\"3.508305\"}");
            if(((ul.getLongitude() == 0) && (ul.getLatitude() == 0)) || (!areaService.isPointInArea(uphf,
                    new Point(ul.getLatitude(), ul.getLongitude()))))
            {
                if(ul.isInZone()) {
                    ul.setInZone(false);
    ...
    public void detectEventAffluence() {
        ...
        for(Area area : areaList) {
            ...
            for(UserLocation uL : userLocations) {
                ...
                if(this.areaService.isPointInArea(area, p)) {
                    count++;
                }
            }
            List<Event> eventList = eventService.findAllByAreaAndEventTypeAndActive(area, type, true);
            //Vérification du dépassement de capacité (= event Affluence)
            Schedule schedule = area.getSchedule();
            int capacity = area.getCapacity();
            if(schedule != null) {
                capacity = schedule.getCount(LocalDate.now().getDayOfWeek(), LocalDateTime.now().getHour());
                capacity *= 1.5;
            }
            if(count > capacity) {
                //Si il n'y a pas déjà un event en cours
                if(eventList.size() == 0) {
                    log.info("[AFFLUENCE]" + area.getName() + " : " + count + "/" + area.getCapacity());
                    //Création d'un évent
